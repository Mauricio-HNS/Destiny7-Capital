...
export class MarketSimulator {
  ...
  private histories: Record<string, MarketTick[]> = {};
  private step = 0;
  ...
  tick(symbol: string): MarketTick {
    ...
    const tick: MarketTick = {
      ...
      timestamp: Date.now(),
      volume: Math.round(1000 + Math.abs(noise) * 250000),
    };
    ...
    this.histories[symbol] = history;

    return tick;
  }

  history(symbol: string, limit = 50): MarketTick[] {
    return [...(this.histories[symbol] ?? [])].slice(-limit);
  }
